...
def test_normalize_entities_merges_duplicates_without_inventing_confidence():
    entities = [
...
def test_build_timeline_sorts_chronologically():
    entities = [
        Entity(type=EntityType.DOMAIN, value="a.com", source="whois",
               status=ResultStatus.CONFIRMED, evidence="Observed date", metadata={"creation_date": "2020-01-01"}),
        Entity(type=EntityType.DOMAIN, value="b.com", source="whois",
               status=ResultStatus.CONFIRMED, evidence="Observed date", metadata={"creation_date": "2010-01-01"}),
    ]
...
def test_compute_risk_handles_empty_investigation():
    ...
    assert any("No confirmed entities" in note for note in risk.notes)
